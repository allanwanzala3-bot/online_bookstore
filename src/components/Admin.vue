...
//map and find are for looping
const allOrders = Object.values(orders).map(order => {
    const book = Object.values(books).find(book => book.id === order.book_id);
    const user = Object.values(users).find(user => user.id === order.customer_id);//id are used to ctreate relationships
  return {
    ...order,
    //allows to add new info on the order
    // ?: tenary operator is the same as a conditional staement
                            ...
                            <v-table class="border">
                                ...
                                <tbody>
                                    <tr v-for="item in allOrders" :key="item.id" >
                                        ...
                                        <td> <v-btn color="warning" size="small"><v-icon icon="mdi-eye" ></v-icon> View</v-btn> </td>
                                        <td> <v-btn color="blue" size="small" @click="editorders (item)"><v-icon icon="mdi-pencil" ></v-icon> Edit </v-btn> </td>
                                        <td> <v-btn color="error" size="small"><v-icon icon="mdi-delete" ></v-icon> Delete</v-btn> </td>
                                    </tr>
                                </tbody>
                            </v-table>
